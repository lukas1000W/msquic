...
// Use the file server ALPN.
//
const QUIC_BUFFER Alpn = { sizeof("file_server") - 1, (uint8_t*)"file_server" };

//
// The UDP port used by the server side of the protocol.
//
const uint16_t UdpPort = 4435;
...
void PrintUsage()
{
    printf(
        "\n"
        "msquicfileserver runs a simple client or server.\n"
        "\n"
        "Usage:\n"
        "\n"
        "  msquicfileserver.exe -server -cert_file:<...> -key_file:<...> -server_files_dir:<...>\n"
        );
}
    ...
    )
{
    ...
    if (GetFlag(argc, argv, "help") || GetFlag(argc, argv, "?")) {
        ...
        // RunClient(argc, argv);
        printf("ERROR: This is a simple File Server! No File Client available yet!\n");
        PrintUsage();
